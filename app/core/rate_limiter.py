...
class RateLimiter:
    ...
    def record_request(self, user_id: str) -> int:
        """Record a request and return the updated count for the current window."""
        current_window = int(time.time()) // self._window_seconds
    ...
    def check_and_record(self, user_id: str) -> tuple[bool, int]:
        """
        Check the limit and record only if allowed.

        Returns (allowed, current_count). Rejected requests are not counted.
        """
