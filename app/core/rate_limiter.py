...
class RateLimiter:
    """Redis sliding-window rate limiter keyed by client identifier."""
    ...
    def record_request(self, user_id: str) -> int:
        """Increment the current window counter and return the new count."""
        current_window = int(time.time()) // self._window_seconds
    ...
    def check_and_record(self, user_id: str) -> tuple[bool, int]:
        """Check the limit and record the request only when allowed."""
