"""Summarization worker — chunked BART summarization with recursive merge."""
...
class SummarizationHandler(BaseJobHandler[dict[str, Any], dict[str, Any]]):
    ...
    def _summarize(self, text: str, depth: int) -> dict[str, Any]:
        ...
        # Recurse when the merged summary is still too long
    ...
    def _chunk_text(self, text: str) -> Generator[str, None, None]:
        """Yield overlapping word windows for long inputs."""
        words = text.split()
        if not words:
            return
        if len(words) <= self._chunk_size:
...
def _get_pipeline():
    """Lazy-load the summarization model once per process."""
    global _pipeline
