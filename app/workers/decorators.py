...
def timed_block(label: str):
    """Time a code block and expose elapsed seconds via the yielded TimerResult."""
...
class TimerResult:
    """Mutable container for elapsed time after a timed_block exits."""
...
def log_execution_time(func: Callable[P, R]) -> Callable[P, R]:
    """Log how long a function took to run."""
...
def monitor_task(func: Callable[P, R]) -> Callable[P, R]:
    """Wrap a Celery task with start/success/failure logging; re-raise errors."""
    @functools.wraps(func)
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
