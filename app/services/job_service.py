...
# Maps job priority to Celery integer priority (lower = higher priority).
# Values match priority_steps in celery_app.py: [0, 5, 9].
PRIORITY_TO_CELERY = {
    JobPriority.HIGH: 0,
...
class JobService:
    ...
    async def async_create_job(
        ...
        """
        Create a job and enqueue it on the job-type Celery queue.

        When input includes file_id, resolve it to an absolute file_path
        and link the JobFile row to the new job.
        """
