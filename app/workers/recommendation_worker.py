"""Collaborative-filtering recommendation handler over user–item interactions."""
import heapq
...
class RecommendationHandler(BaseJobHandler[dict[str, Any], dict[str, Any]]):
    """Recommend unseen items from users with overlapping interaction history."""
    ...
    def process(self, input_payload: dict[str, Any]) -> dict[str, Any]:
        """Build interaction graph, score candidates, and return top-K items."""
    ...
    def _score_candidates(
        ...
        """Score unseen items from similar users by similarity × rating."""
