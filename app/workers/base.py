...
class BaseJobHandler(ABC, Generic[InputT, ResultT]):
    """Contract that every job-type handler must implement."""
    ...
    def format_result(self, raw_result: ResultT) -> dict[str, Any]:
        """Normalize the raw result before it is stored in result_payload."""
        ...

    def run(self, input_payload: InputT) -> dict[str, Any]:
        """Validate, process, then format — subclasses override the steps, not run()."""
