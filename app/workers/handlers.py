"""Maps JobType to its BaseJobHandler instance."""
...
def _get_or_create(job_type: JobType) -> BaseJobHandler:
    """Return a cached handler for the job type, creating it on first use."""
...
def get_handler(job_type: JobType):
    """Return handler.run for the given job type (input_payload → result dict)."""
