"""Transcription worker for timestamped audio segments."""
...
class TranscriptSegment:
    """Immutable segment of transcribed audio with start/end timestamps."""
    start: float
    end: float
    text: str
...
class TranscriptionHandler(BaseJobHandler[dict[str, Any], dict[str, Any]]):
    """Transcribes audio into timestamped text segments (simulated until Whisper is wired)."""
    ...
    def process(self, input_payload: dict[str, Any]) -> dict[str, Any]:
        """Chunk audio timeline, transcribe segments, merge overlaps, and align timestamps."""
    ...
    def _generate_time_chunks(
        ...
        """Yield overlapping (start, end) time windows across the audio duration."""
    ...
    def _merge_overlapping_segments(
        ...
        """Merge overlapping transcript segments into a non-overlapping timeline."""
        if not segments:
            return []
    ...
    def _merge_texts(text_a: str, text_b: str) -> str:
        """Merge overlapping segment text without duplicating shared words."""
